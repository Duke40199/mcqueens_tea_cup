...
func (h *Handler) HandleOBMeta(i *discordgo.InteractionCreate, optMap map[string]string) {
	...
	for j := 0; j < limit; j++ {
		...
		if specStyleMap[baseSpec][style][modelCode] == nil {
			specStyleMap[baseSpec][style][modelCode] = &CarStat{Name: getCarStatName(&specInfo), Count: 0}
		}
	...
	header := fmt.Sprintf(
		"# Initial DAC Car Meta (Online Battle)\n"+
			...
			"### Calculated at: %s (JST time, local time coming soon.)\n\n",
		areaName, currentRound, totalSampled, records.CalcDate)
	currentMessage.WriteString(header)
...
func getCarStatName(carStat *entity.CarSpecInfo) string {
	if carStat.Maker == "" || carStat.CarName == "" {
		return carStat.ModelCode
	}
